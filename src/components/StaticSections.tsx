...
export function SectionHeading({
  ...
  return (
    <div className={`max-w-3xl ${alignment}`}>
      <p className={`section-kicker ${inverse ? "!text-[#d8b48d]" : ""}`}>
        {eyebrow}
      </p>
...
export function HeroSection({
  ...
  return (
    <section className="grain relative overflow-hidden pb-16 pt-8 md:pb-24 md:pt-14">
      <div
        className="pointer-events-none absolute -right-40 top-10 h-96 w-96 rounded-full bg-[#dce2c8]/70 blur-3xl"
        aria-hidden="true"
      />
      <div className="site-container relative grid items-center gap-10 lg:grid-cols-[1.02fr_0.98fr] lg:gap-14">
        <div>
          <div className="mb-5 flex flex-wrap items-center gap-3">
            <span className="inline-flex rounded-full border border-[#173f2b]/15 bg-white/70 px-3 py-1 text-xs font-extrabold tracking-[0.1em] text-[#173f2b] uppercase">
              {content.eyebrow}
            </span>
            <span
              className={`inline-flex rounded-full px-3 py-1 text-xs font-extrabold tracking-[0.08em] uppercase ${
                status === "available"
                  ? "bg-[#dfe8d3] text-[#173f2b]"
                  : "bg-[#f0dfcc] text-[#7d4927]"
              }`}
            >
              {statusContent.label}
            </span>
        ...
        <div>
          <figure className="image-frame aspect-[3/2]">
            <Image
          ...
          {content.quickFacts.length > 0 ? (
            <ul className="relative z-10 mx-3 -mt-5 grid gap-px overflow-hidden rounded-2xl border border-[#173f2b]/10 bg-[#173f2b]/10 shadow-[0_18px_50px_rgba(23,63,43,0.13)] sm:grid-cols-2 lg:mx-6">
              {content.quickFacts.map((fact) => (
                <li
                  key={fact}
                  className="flex min-h-20 items-center gap-3 bg-white px-4 py-3 text-sm font-bold text-[#173f2b]"
                >
                  <Check aria-hidden="true" className="shrink-0 text-[#a96531]" size={18} />
                  {fact}
                </li>
              ))}
            </ul>
            ...
            {visibleHighlights.map((item) => {
              ...
              return (
                <li key={item.title} className="card-surface p-6">
                  <span className="mb-5 grid size-11 place-items-center rounded-xl bg-[#e7ead8] text-[#173f2b]">
                    <Icon aria-hidden="true" size={23} strokeWidth={1.8} />
                  </span>
                  <h3 className="text-lg font-extrabold text-[#173f2b]">{item.title}</h3>
              ...
              <article key={crop.id} className="card-surface overflow-hidden bg-white">
                ...
                <div className="p-6 md:p-7">
                  <div className="mb-4 flex items-center gap-3">
                    <span className="grid size-10 place-items-center rounded-full bg-[#e7ead8] text-[#173f2b]">
                      <Wheat aria-hidden="true" size={20} />
                    </span>
                    <h3 className="font-serif text-3xl font-semibold tracking-[-0.03em] text-[#173f2b]">
                      {crop.name}
                    </h3>
                  </div>
...
export function ProductivePotentialSection({
  ...
  return (
    <section id={content.id} className="section-space relative overflow-hidden bg-[#173f2b] text-white">
      ...
      <div className="site-container relative">
        ...
        <div className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 lg:grid-cols-3">
          {items.map((item, index) => {
            const Icon = iconByName[item.icon];
            return (
              <article key={item.title} className="bg-[#173f2b] p-7 md:p-9">
                <div className="mb-8 flex items-center justify-between">
                  <span className="grid size-12 place-items-center rounded-xl bg-white/10 text-[#d8b48d]">
        ...
        <p className="mt-7 flex max-w-3xl items-start gap-3 text-sm leading-6 text-white/62">
          <ShieldCheck aria-hidden="true" className="mt-0.5 shrink-0 text-[#d8b48d]" size={19} />
          {content.disclaimer}
        </p>
      </div>
    </section>
  ...
  return (
    <section id={content.id} className="section-space bg-white">
      <div className="site-container grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-[#173f2b] p-7 text-white shadow-[0_24px_70px_rgba(23,63,43,0.18)] md:p-9">
          <div className="absolute -bottom-20 -right-16 size-64 rounded-full border-[45px] border-white/5" />
          <span className="grid size-14 place-items-center rounded-2xl bg-white/10 text-[#d8b48d]">
    ...
    <section id={content.id} className="section-space bg-white">
      <div className="site-container overflow-hidden rounded-[2rem] border border-[#173f2b]/10 bg-[#f5f1e8] shadow-[0_24px_70px_rgba(23,63,43,0.1)]">
        <div className="grid lg:grid-cols-[0.72fr_1.28fr]">
          <div className="relative flex min-h-64 items-center justify-center overflow-hidden bg-[#173f2b] p-10 text-white">
            <div className="absolute -bottom-24 -left-20 size-72 rounded-full border-[55px] border-white/5" />
            <div className="relative text-center">
              <span className="mx-auto grid size-20 place-items-center rounded-full border border-white/15 bg-white/10 text-[#d8b48d]">
...
export function FinalCTASection({
  ...
  return (
    <section id={content.id} className="relative isolate overflow-hidden bg-[#10301f] py-24 text-white md:py-32">
      <Image
      ...
      />
      <div className="absolute inset-0 -z-10 bg-[linear-gradient(90deg,rgba(16,48,31,.98)_0%,rgba(16,48,31,.84)_55%,rgba(16,48,31,.63)_100%)]" />
      {content.backgroundImage.isPlaceholder ? <PlaceholderBadge /> : null}
      <div className="site-container">
        <div className="max-w-3xl">
          <MessageCircle aria-hidden="true" className="mb-6 text-[#d8b48d]" size={36} strokeWidth={1.7} />
...
export function Footer({
  ...
  return (
    <footer className="bg-[#0d291b] pb-28 pt-14 text-white md:pb-10">
      <div className="site-container">
